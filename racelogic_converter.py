...
def generate_synthetic_track_data(cfg_file, track_info, xml_db):
    """Generate synthetic track data when no real data is available"""
    log_message(f"Generating synthetic track data from: {cfg_file}")
    config = parse_config_file(cfg_file)
    track_name = track_info.get("name", "Unknown Track")

    # Check if there's a PNG file that we can use to get track outline
    png_path = None
    if "background" in config:
        png_file = config["background"]
        # Replace PNGIMAGES with the actual path
        trackmaps_dir = os.path.dirname(cfg_file)
        country_dir = os.path.dirname(trackmaps_dir)
        png_dir = os.path.join(country_dir, "PNGIMAGES")
        potential_png = os.path.join(png_dir, os.path.basename(png_file))

        if os.path.exists(potential_png):
            png_path = potential_png
            log_message(f"Found PNG file: {png_path}")

    # If we have min/max coordinates from the track info, use them to create a better outline
    if "min" in track_info and "max" in track_info:
        try:
            log_message(f"Using min/max coordinates: min={track_info['min']}, max={track_info['max']}")
            # Parse min/max coordinates (format: "min_long,min_lat", "max_long,max_lat")
            min_coords = track_info["min"].split(",")
            max_coords = track_info["max"].split(",")

            if len(min_coords) == 2 and len(max_coords) == 2:
                min_long = float(min_coords[0])
                min_lat = float(min_coords[1])
                max_long = float(max_coords[0])
                max_lat = float(max_coords[1])

                # Determine hemisphere
                is_eastern = get_cfg_hemisphere(cfg_file)
                if is_eastern is None:
                    # Fall back to min_long in database if CFG doesn't define hemisphere
                    west_hemisphere = min_long < 0
                else:
                    west_hemisphere = not is_eastern
                
                log_message(f"Min/max based hemisphere: {'Western' if west_hemisphere else 'Eastern'}")

                # Convert to decimal degrees
                min_long_deg = -convert_to_decimal_degrees(abs(float(min_long))) if west_hemisphere else convert_to_decimal_degrees(float(min_long))
                min_lat_deg = convert_to_decimal_degrees(float(min_lat))
                max_long_deg = -convert_to_decimal_degrees(abs(float(max_long))) if west_hemisphere else convert_to_decimal_degrees(float(max_long))
                max_lat_deg = convert_to_decimal_degrees(float(max_lat))

                log_message(f"Decimal degrees: min=({min_lat_deg},{min_long_deg}), max=({max_lat_deg},{max_long_deg})")

                # Create a rectangle track using the min/max bounds
                points = []
                # Bottom edge
                for i in range(0, 10):
                    long = min_long_deg + (max_long_deg - min_long_deg) * i / 9
                    points.append(
                        {
                            "lat": min_lat_deg,
                            "long": long,
                ...
                # Right edge
                for i in range(0, 10):
                    lat = min_lat_deg + (max_lat_deg - min_lat_deg) * i / 9
                    points.append(
                        {
                            "lat": lat,
                            "long": max_long_deg,
                ...
                # Top edge
                for i in range(0, 10):
                    long = max_long_deg - (max_long_deg - min_long_deg) * i / 9
                    points.append(
                        {
                            "lat": max_lat_deg,
                            "long": long,
                ...
                # Left edge
                for i in range(0, 10):
                    lat = max_lat_deg - (max_lat_deg - min_lat_deg) * i / 9
                    points.append(
                        {
                            "lat": lat,
                            "long": min_long_deg,
                ...
                log_message(f"Generated rectangle track with {len(points)} points")
                return points
        except Exception as e:
            log_message(f"Error creating track from min/max coordinates: {e}")
            pass

    # If we have start/finish coordinates, create a simple track
    if track_info.get("splitinfo"):
        # Find start/finish coordinates
        start_finish = None
        for split in track_info["splitinfo"]:
            if (
                split.get("name") == "Start / Finish"
                or split.get("name") == "Start/Finish"
            ):
                start_finish = split
                break

        if start_finish and "lat" in start_finish and "long" in start_finish:
            log_message(f"Using start/finish point: {start_finish}")
            # Create synthetic data points around the start/finish point
            lat = float(start_finish["lat"].replace("+", ""))
            long = float(start_finish["long"].replace("-", "").replace("+", ""))

            # Determine hemisphere from CFG file
            is_eastern = get_cfg_hemisphere(cfg_file)
            if is_eastern is None:
                # If CFG doesn't define hemisphere, use start/finish sign
                west_hemisphere = start_finish["long"].startswith("-")
            else:
                west_hemisphere = not is_eastern
                
            log_message(f"Start/finish based hemisphere: {'Western' if west_hemisphere else 'Eastern'}")
            
            # Convert to decimal degrees
            lat_deg = convert_to_decimal_degrees(lat)
            long_deg = -convert_to_decimal_degrees(long) if west_hemisphere else convert_to_decimal_degrees(long)
            log_message(f"Start/finish decimal degrees: ({lat_deg},{long_deg})")

            # Get the length if available, or use a default
            length = float(track_info.get("length", 2000))
            log_message(f"Track length: {length}m")

            # Create a simple oval track centered on the start/finish point
            radius = (
                length / (2 * math.pi) / 111000
            )  # Convert meters to degrees (approx)
            log_message(f"Calculated radius: {radius} degrees")
            ...
            for i in range(0, 360, 10):
                ...
                y_offset = math.sin(angle) * radius * 0.5  # Make it an oval

                points.append(
                    {
                        "lat": lat_deg + y_offset,
                        "long": long_deg + x_offset,
                        "height": 0,
                        "velocity": 100,
                        "heading": (i + 90) % 360,
                    }
            ...
            log_message(f"Generated oval track with {len(points)} points")
            return points

    # Also try to use base coordinates from the CFG file if available
    if "base_latitude" in config and "base_longitude" in config:
        try:
            base_lat = float(config["base_latitude"].replace("+", ""))
            base_long = float(
                config["base_longitude"].replace("-", "").replace("+", "")
            )
            log_message(f"Using base coordinates: lat={base_lat}, long={base_long}")

            # Determine hemisphere directly from the CFG file base_longitude sign
            is_eastern = config["base_longitude"].startswith("+")
            west_hemisphere = not is_eastern
            
            log_message(f"Base coordinates hemisphere: {'Western' if west_hemisphere else 'Eastern'}")

            # Convert to decimal degrees
            base_lat_deg = convert_to_decimal_degrees(base_lat)
            base_long_deg = -convert_to_decimal_degrees(base_long) if west_hemisphere else convert_to_decimal_degrees(base_long)
            log_message(f"Base decimal degrees: ({base_lat_deg},{base_long_deg})")

            # Get track length estimate
            length = 3000  # Default length in meters
            if "map_region" in config:
                # Parse map region (format: "(+lat,-long,+lat,-long)")
                map_region = config["map_region"].strip("()").split(",")
                if len(map_region) == 4:
                    # Use the map region to estimate track size
                    lat1 = float(map_region[0].replace("+", ""))
                    long1 = float(map_region[1].replace("-", "").replace("+", ""))
                    lat2 = float(map_region[2].replace("+", ""))
                    long2 = float(map_region[3].replace("-", "").replace("+", ""))
                    ...
                    width = (
                        ...
                        / 60
                        * 111000
                        * math.cos(math.radians(base_lat_deg))
                    )
                    height = abs(lat1 - lat2) / 60 * 111000

                    # Use perimeter as length estimate
                    length = 2 * (width + height)
                    log_message(f"Estimated length from map region: {length}m")

            # Create a simple oval track centered on the base point
            radius = (
                length / (2 * math.pi) / 111000
            )  # Convert meters to degrees (approx)
            log_message(f"Calculated radius: {radius} degrees")
            ...
            for i in range(0, 360, 10):
                ...
                points.append(
                    {
                        "lat": base_lat_deg + y_offset,
                        "long": base_long_deg + x_offset,
                        "height": 0,
                        "velocity": 100,
                        "heading": (i + 90) % 360,
                    }
            ...
            log_message(f"Generated base-centered oval track with {len(points)} points")
            return points
        except Exception as e:
            log_message(f"Error creating track from base coordinates: {e}")
            pass

    # If no real or synthetic data could be created, return an empty list
    log_message(f"Failed to generate synthetic track data")
    return []
...
def generate_kml(parsed_data, track_info):
    ...
    # Check if we have start/finish line coordinates in
...
def process_track(track_params):
    ...
    try:
        ...
        # If we couldn't parse the data file or it had no valid points, try synthetic data
        if not parsed_data and cfg_files and track_info:
            log_message(f"No valid parsed data, trying synthetic data")
            parsed_data = generate_synthetic_track_data(cfg_files[0], track_info, xml_db)
